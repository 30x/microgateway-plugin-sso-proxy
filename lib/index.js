...
const shipyardPrivateSecret = process.env.SHIPYARD_PRIVATE_SECRET !== undefined ? new Buffer(process.env.SHIPYARD_PRIVATE_SECRET).toString('base64') : undefined
  ...
  function onrequest(req, res, data, next) {
    ...
    if (req.method === 'GET' && req.reqUrl.pathname === oauthCallbackPath) {
      authenticate(req, res, err => {
        ...
        if (!redirectPath) {
          if (shipyardPrivateSecret) req.headers['x-routing-api-key'] = shipyardPrivateSecret
